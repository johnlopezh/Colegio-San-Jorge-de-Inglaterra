...
class ulp_userengage_class {
	...
	function popup_options_show($_popup_options) {
		...
		echo '
				...
				<table class="ulp_useroptions">
					<tr>
						<th colspan="2" style="color: red;">'.__('UserEngage API is still under development. At that moment it does not allow to update user details. Only new user can be created through their API.', 'ulp').'</th>
					</tr>
					<tr>
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['userengage_enable'] == 'on') {
			...
			if (array_key_exists('id', $result)) $contact_id = $result['id'];
