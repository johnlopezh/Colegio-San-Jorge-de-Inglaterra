				...
				if ($subscriber_details) {
					...
					if (empty($popup_details)) {
						...
						$subscriber = array(
							...
							'{confirmation-link}' => get_bloginfo('url').'?ulp-confirm='.$confirmation_id
						);
...
<body>
	...
	function front_init() {
		...
		if (is_singular() || $posts_page || $woo_page) {
			...
			var ulp_onscroll_offset = "'.($meta['onscroll_popup'] == 'default' ? intval($ulp->options['onscroll_offset']) : intval($meta['onscroll_offset'])).'";
			...
			var ulp_onscroll_popup = "'.$onscroll_popup.'";
			var ulp_onscroll_offset = "'.intval($ulp->options['onscroll_offset']).'";
			...
			foreach ($layers as $layer) {
				...
				$content = str_replace(
					...
					array(
						...
						'<a class="ulp-submit'.($popup_options['button_inherit_size'] == 'on' ? ' ulp-inherited' : '').'" onclick="return ulp_subscribe(this);"'.(!empty($button_icon) ? ' data-icon="'.$button_icon.'"' : '').' data-label="'.esc_html($popup_options['button_label']).'" data-loading="'.esc_html($popup_options['button_label_loading']).'">'.(!empty($button_icon) ? '<i class="fa '.$button_icon.'"></i>' : '').(!empty($button_icon) && !empty($popup_options['button_label']) ? '&nbsp; ' : '').esc_html($popup_options['button_label']).'</a>'),
